...
基于单目视频的人体运动捕捉与测量系统。输入任意视频，输出人体33个关节点的运动轨迹、关节角度/角速度/角加速度时序数据、篮球物理运动参数，并支持纯黑背景的可视化回放。
...
4. 追踪篮球位置，计算瞬时速度、累计位移
5. 提供图形界面进行交互式录制与纯黑背景回放
6. 导出所有测量结果为CSV/JSON/MAT格式
...
- **篮球追踪**：HSV色彩空间双通道掩膜（粉色H:155-180 + 红色H:0-10）+ 形态学清洗 + CSRT容错回退
- **运动学计算**：余弦定理（角度）、中心差分法（角速度/角加速度）、累计欧氏距离（轨迹长度）
- **GUI框架**：PyQt5，QThread后台处理防止界面卡死
...
2. 点击 **Start**，后台开始 MediaPipe 姿态估计 + 篮球追踪，前台显示原视频
3. 视频结束或点击 **End**，进入回放就绪状态
4. 点击 **Replay**，纯黑背景回放：
   - 青色(Cyan)线条和点：人体骨架运动姿态
   - 红色实心圆点：篮球当前位置
   - 黄色连续线条：篮球历史轨迹
   - 左上角实时显示：`Velocity(mm/s)` `Distance(mm)` `Times(s)`
5. 点击 **Save Result**，将回放动画导出为 MP4 文件
...
篮球的瞬时速度通过对最近5帧的帧间位移做移动平均平滑后除以 $\Delta t$ 得到，以抑制像素抖动造成的速度突刺：
